...
public static class AgentConfigLoader
{
    public static AgentOptions Load(string path)
    {
        ...
        if (yaml.Documents.Count != 1 || yaml.Documents[0].RootNode is not YamlMappingNode root)
            throw new InvalidOperationException("MEŽS configuration must contain one YAML mapping document.");
        ...
        if (!int.TryParse(
                ...
                out var version) ||
            version != 1)
            throw new InvalidOperationException($"Unsupported config version {versionText}.");

        var extensions = RequiredMapping(root, "extensions", "extensions");
        var agent = RequiredMapping(extensions, "agent", "extensions.agent");
        EnsureOnlyKeys(
            agent,
            "extensions.agent",
            "listen",
        ...
        {
            Listen = RequiredHttpUri(agent, "listen", "extensions.agent.listen"),
            MezhsApi = RequiredHttpUri(agent, "mezhsApi", "extensions.agent.mezhsApi"),
            Storage = Resolve(
                configDirectory,
        ...
        params string[] allowed)
    {
        ...
        foreach (var keyNode in mapping.Children.Keys)
        {
            ...
            if (!allowedSet.Contains(key.Value))
                throw new InvalidOperationException($"Unknown configuration property '{path}.{key.Value}'.");
        }
    }
    ...
    private static Uri RequiredHttpUri(
        YamlMappingNode parent,
        string key,
        string path)
    {
