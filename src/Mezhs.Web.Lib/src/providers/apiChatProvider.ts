...
export class ApiChatProvider implements ChatProvider {
  ...
  async initialize(): Promise<void> {
    ...
    await apiJson(
      this.apiBase,
      `/v1/connections/${encodeURIComponent(this.connection.id)}/login`,
      { method: "POST" },
    );
  }
  ...
  async getModels(): Promise<ConnectionModel[]> {
    ...
    const response = await apiFetch(
      this.apiBase,
      `/v1/connections/${encodeURIComponent(this.connection.id)}/models`,
    );
  ...
  async getChat(chatId: string): Promise<Chat> {
    const [chat, messages] = await Promise.all([
      apiJson<Chat>(this.apiBase, `/v1/chats/${encodeURIComponent(chatId)}`),
      apiJson<Chat["messages"]>(
        this.apiBase,
  ...
  async createChat(options?: CreateChatOptions): Promise<Chat> {
    return apiJson<Chat>(this.apiBase, "/v1/chats", {
      method: "POST",
  ...
  async sendMessage(
    ...
    return apiJson<ChatResponse>(this.apiBase, "/v1/messages", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async uploadFile(file: FileInput, _options?: UploadOptions): Promise<UploadedFile> {
    ...
    return apiJson<UploadedFile>(this.apiBase, "/v1/files", {
      method: "POST",
      body: form,
    });
  }
  ...
  async downloadFile(_chatId: string, fileId: string): Promise<DownloadedFile> {
    const file = await apiJson<ApiFile>(
      this.apiBase,
      `/v1/files/${encodeURIComponent(fileId)}`,
    );
    const response = await apiFetch(this.apiBase, file.downloadUrl);
