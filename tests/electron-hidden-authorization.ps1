...
try {
    ...
    if ($stdout -notmatch 'authorization is required' -or $stdout -notmatch 'login action') {
        throw "Electron did not fail toward the explicit login path. stdout=$stdout"
    }

    Write-Host 'PASS: hidden authorization stays hidden and fails toward the explicit login action.'
}
finally {
