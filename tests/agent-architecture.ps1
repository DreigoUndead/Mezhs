...
$agentProject = Read "src/Mezhs.Agent.Api/Mezhs.Agent.Api.csproj"
$coreComposition = Read "src/Mezhs.Api/MezhsApi.cs"
$agentWebHost = Read "src/Mezhs.Agent.Web/Program.cs"
$agentWebApp = Read "src/Mezhs.Agent.Web/src/App.tsx"
$chatStore = Read "src/Mezhs.Api/Services/ChatStore.cs"
$executorStore = Read "src/Mezhs.Executor/ExecutorStore.cs"
$executorRunner = Read "src/Mezhs.Executor/ExecutorRunner.cs"
$executorPlatform = Read "src/Mezhs.Executor/ExecutorPlatform.cs"
...
Assert ($executorRunner -match 'ProcessStartInfo' -and $executorRunner -match 'platform\.PrepareShell\(execution\.Command\)' -and $executorRunner -match 'RedirectStandardInput = shell\.StandardInput is not null' -and $executorPlatform -match 'PrepareShell\(string command\)') "Executor does not own host shell process creation and platform-specific command delivery."
...
Assert ($agentWebApp -notmatch 'setInterval' -and $agentWebApp -match 'refreshRuntime' -and $agentWebApp -match 'selectedLoadChatRef' -and $agentWebApp -match 'setTimeout\(\(\) => void poll\(\)') "Agent dashboard polling can overlap or still reload full chat state on a fixed interval."
...
Assert ($program -notmatch 'messages\.ActiveMessage\.Analysis' -and $agentWebApp -notmatch 'activity\.analysis') "Agent runtime polling still transports the growing model analysis payload."
...
Assert ($store -match 'GetFirstRootExecutionRequest' -and $store -match 'substr\(Request, 1, 200\)' -and $program -match 'GetFirstRootExecutionRequest\(record\.ChatId\)') "Agent chat list still loads an unbounded execution request just to derive the title."
Assert ($chatStore -match 'TryGetListState' -and $chatStore -match 'Length: > 200' -and $program -match 'chats\.TryGetListState\(record\.ChatId\)') "Agent chat list still materializes unbounded transcript content for sidebar metadata."
